...
WITH tbl AS (
	SELECT t.id_transaccion, t.id_cliente, t.numeroComprobante, t.puntoVenta, t.id_tipoComprobante, t.fecha, 
	...
	WHERE t.id_cliente = @id_cliente AND t.id_cc = @id_cc AND t.fecha BETWEEN @fecha_inicio AND @fecha_fin-- AND t.activo = 0 AND t.cerrado = 1 
	--AND t.esTest = 0 AND t.esPresupuesto = 0   
	--AND cmp.id_tipoComprobante IN (1, 2, 3, 4, 6, 7, 8, 9, 11 ,12, 13, 15, 51, 52, 53, 54, 200, 201)   
) 
SELECT tbl.id_transaccion AS 'ID', tbl.fecha AS 'Fecha'
	, dbo.CalculoComprobante(tbl.id_tipoComprobante, tbl.puntoVenta, tbl.numeroComprobante) AS 'Comprobante', 
...
WHERE tbl.id_cliente = @id_cliente AND tbl.id_cc = @id_cc AND tbl.fecha BETWEEN @fecha_inicio AND @fecha_fin 
